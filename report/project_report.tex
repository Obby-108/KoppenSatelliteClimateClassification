...
We then plan on using our models, such as CNNs and SVMs, to learn the patterns between spectral bands and climate classes, applying case-by-case model preprocessing to ensure the data is understandable to each model.
For example, SVM cannot analyze data that is not flattened, so we might apply feature extraction by calculating average wavelength intensity for a given spectral band.
...
From here, training the SVM was a simple matter of passing in the data to the chosen \texttt{NuSVC} model—which limits the number of support vectors—and fitting it on it.
We experimented with different kernels, but RBF proved to be the best.
As expected, the SVM only achieved 41\% testing accuracy, due to the data being simplified aggressively in order for it to be usable by the SVM.
...
In the future, this project could benefit from various improvements, including exploring benefits from using lighter models such as Gradient Boosted Classifiers and Random Forests, which were covered in \texttt{ensemble.ipynb}.
